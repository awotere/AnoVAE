...
class AnoVAE:
    ...
    def Train(self, path=None):
        ...
        # W保存
        name = pyautogui.prompt(text="weight保存名を指定してください", title="AnoVAE", default="ts{0}_id{1}_ld{2}_b{3}".format(G.NUM_TIMESTEPS,G.INTERMIDIATE_DIM,G.LATENT_DIM,G.BATCH_SIZE))
    ...
    def TestCSV(self, path=None):
        ...
        for i in range(len(true)):
            ...
            sum = 0
            for j in range(max(0, i - G.NUM_TIMESTEPS), i):
